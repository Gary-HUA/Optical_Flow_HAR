...
clear;
clc;
% feature extraction function from Weizmann datasets
% ACTION: WALK, BEND, JACK, PJUMP, RUN, SIDE, SKIP, WAVE1, WAVE2.

[file,path] = uigetfile('Train_Datasets\walk\denis_walk.avi');
filename=fullfile(path,file);
obj = VideoReader(filename);
numFrames = obj.NumberOfFrames;% frame id
save_resx = zeros(1,144); % feature vector 


width = obj.Width;
height =obj.Height; 
Threshold = 0.007; CloseSize = 20;  % key parameter for proper features vector.values in parameter.txt
desiredAngles = 5:5:360;
DisAngles = 45:45:360;
heiwid = numel(obj);
boundary = ones(height,width);
label = 1; % label of data 1 to 9


% Guassian mixture model :foreground
%creat a detector object
detector = vision.ForegroundDetector( ...
    'NumTrainingFrames',5, 'InitialVariance', 5*5);
% Perform blob analysis.
blob = vision.BlobAnalysis(...
       'CentroidOutputPort', false, 'AreaOutputPort', false, ...
       'BoundingBoxOutputPort', true, ...
       'MinimumBlobAreaSource', 'Property', 'MinimumBlobArea', 700,...
  'MaximumCount',2);
% insert a bounder
shapeInserter = vision.ShapeInserter('BorderColor','Custom');
opticFlow = opticalFlowHS('Smoothness',7, 'MaxIteration', 10, 'VelocityDifference', 0);
...
frameLogical = ones(height,width);
 for k = 1 : numFrames 
    ...
    % frameLogical = logical(frameLogical);% binarize frame logical type
            ...
            out = insertShape(frameGray,'Rectangle',bbox,'color','green');% output with boundary on frame
...
Result_Ed = sqrt((eixe-xCenter).^2+(eiye-yCenter)).^2;
...
%dlmwrite('F:\Optical_code\Speed_EightDis_TestSIDE.csv',result_final_L,'-append');
